        ...
        else if (last->isUncond())
        {
            ...
            {//相应dst为空，就根据函数返回类型插入ret指令
                if (((FunctionType*)(se->getType()))->getRetType() == TypeSystem::intType)
                    new RetInstruction(new Operand
                    (new ConstantSymbolEntry(TypeSystem::intType, 0)), dst);
    ...
    if (se->isGlobal()) {
        ...
        if (expr) {
            if (expr->isInitValueListExpr()) {
                ...
                std::vector<int> idx;
                idx.push_back(0);//index存储的为我们每一维度的元素个数,刚开始都为0
                while (temp) {
                    if (temp->isInitValueListExpr()) {
                        ...
                        continue;
                    }//大括号的嵌套的深度,一直从最高维到最低维度
                    else {
                        temp->genCode();//当到达了最后一维度了，此时InitValueListExpr存储的就是待分配的值
                        Type* type =
                            ((ArrayType*)(se->getType()))->getElementType();//取元素类型
                        Operand* tempSrc = addr;
                        Operand* tempDst;
                        Operand* index;
                        bool flag = true;
...
void CallExpr::genCode() {
    ...
    while (temp) {
        temp->genCode();
        operands.push_back(std::move(temp->getOperand()));
        temp = ((ExprNode*)temp->getNext());
    }
    BasicBlock* bb = builder->getInsertBB();
...
void ImplictCastExpr::genCode() {
    ...
    this->trueList().push_back(
        std::move(new CondBrInstruction(trueBB, tempbb, this->dst, bb)));
    //压入指令：无条件跳转到假块
    this->falseList().push_back(std::move(new UncondBrInstruction(falseBB, tempbb)));
}